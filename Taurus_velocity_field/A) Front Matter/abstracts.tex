...
with a power spectrum inferred from the data. A star without a radial velocity still constrains the field within the plane
of the sky, while its missing component is supplied by the spatial correlation with its neighbors. The input catalog consists of 557 members identified with \texttt{SigMA} in
...
but does so by widening the uncertainties rather than by improving the field.
