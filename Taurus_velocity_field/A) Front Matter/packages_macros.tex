...
\usepackage{subcaption} % for side-by-side/stacked figures with sub-captions
\usepackage{booktabs}   % \toprule, \midrule, \cmidrule, \bottomrule in tables

\usepackage[numbers]{natbib}
% abbrvnat.bst already prints a \doi{...} for every bib entry with a "doi"
% field; its fallback definition only styles the text, it does not actually
% link to doi.org. Defining \doi here (before the .bbl's \providecommand)
% overrides that fallback so every entry gets a real, clickable doi.org link.
